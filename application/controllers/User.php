...
class User extends CI_Controller {
    ...
    public function upost() {
        ...
        $plasa = $this->input->post('plasa');

        if ($id == NULL) {
            $additional_data = array(
                'full_name' => $full_name,
                'lokasi_layanan' => $plasa,
            );
...
//        $data['users'] = $this->ion_auth->users()->result();
        ...
        $data['users'] = $this->Plasa_model->get_data();
//        $data['v'] = 'user/list';
        $this->load->view('user/list', $data);
    }

    public function ulist_witel() {
        $kobar  = $this->input->post('witel');
        $data['userdata'] = $this->ion_auth->user()->row();
//        $data['users'] = $this->ion_auth->users()->result();
        $data['plasa'] = $this->Plasa_model->get_plasa();
        $data['users'] = $this->Plasa_model->get_data_witel($kobar);
//        $data['v'] = 'user/list';
        $this->load->view('user/list', $data);
    }

    public function ulist_plasa() {
        $kobar  = $this->input->post('plasa');
        $data['userdata'] = $this->ion_auth->user()->row();
//        $data['users'] = $this->ion_auth->users()->result();
        $data['plasa'] = $this->Plasa_model->get_plasa();
        $data['users'] = $this->Plasa_model->get_data_plasa($kobar);
//        $data['v'] = 'user/list';
        $this->load->view('user/list', $data);
    }

    public function gnew() {
...
//        $data['v'] = 'user/listgroup';
        $this->load->view('user/listgroup', $data);
    }
